...
export interface ReportSpec {
  ...
  // Premium report branding — a custom logo shown in place of the Volteq mark.
...
// Header brand mark: logo only, never text — a Premium user's own uploaded
// logo replaces the default black Volteq lockup (icon + wordmark already
...
function buildPrintableDom(spec: ReportSpec): HTMLDivElement {
  ...
  const brandMarkHtml = renderBrandMark(spec);
  const now = new Date();
  ...
  root.innerHTML = `
    <div style="padding:28px 32px;">
      <div style="display:flex; justify-content:space-between; align-items:baseline; border-bottom:2px solid ${accent}; padding-bottom:10px; margin-bottom:14px;">
        <div>
          ${brandMarkHtml}
          <div style="font-size:16px; font-weight:700; margin-top:6px;">${escapeHtml(spec.pageTitle)}</div>
        </div>
      ...
      <div style="margin-top:8px; font-size:8px; color:#9A9D95;">Generated by <span style="color:${accent}; font-weight:600;">Volteq.io</span></div>
    </div>
